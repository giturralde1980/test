...
- **Ver el reporte HTML desde Jenkins no funciona bien directo en el navegador** — Jenkins aplica una Content-Security-Policy a los artifacts servidos que bloquea la ejecución de JS embebido, así que al abrir el `index.html` desde el visor de artifacts se ve el código fuente crudo en pantalla en vez del reporte. Y si se descarga y se abre como archivo local (doble-click), queda en blanco por el mismo motivo de CORS explicado en la sección 2. La forma de verlo bien: descargar la carpeta del reporte del build y correr `npx playwright show-report <carpeta>` localmente. Solución permanente pendiente: plugin **HTML Publisher** de Jenkins (necesita admin).
- **Con `REGION=all` el reporte de la primera región se pisaba con el de la segunda — resuelto (2026-08-07).** Los stages `Andalucía` y `Madrid` corren dentro del **mismo workspace** de Jenkins (a diferencia de Bitbucket Pipelines, donde cada `step` es un contenedor aislado con su propio artifact). Como ambos usan el mismo `playwright.config.ts` con las rutas de reporte fijas (`reports/html`, `reports/junit/results.xml`), el segundo `npm run test:*` sobreescribía por completo lo que había generado el primero — el artifact final solo tenía la última región que corrió. Se arregló seteando `PLAYWRIGHT_HTML_REPORT` y `PLAYWRIGHT_JUNIT_OUTPUT_FILE` (variables de entorno que Playwright respeta y pisan la config) distintas por stage en el `Jenkinsfile` (`reports/html-andalucia` / `reports/html-madrid`, y sus respectivos JUnit) — sin tocar `playwright.config.ts` ni las otras pipelines, que no tienen este problema.
